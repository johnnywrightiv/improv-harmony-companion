...
const alertVariants = cva(
	'relative w-full rounded-[--radius] border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground',
	{
		variants: {
			variant: {
				default: 'bg-muted-foreground/10 text-foreground border-text/50',
				destructive:
...
>(({ className, ...props }, ref) => (
	<div
		ref={ref}
		className={cn(
			'text-sm text-black dark:text-white [&_p]:leading-relaxed',
			className
		)}
		{...props}
	/>
